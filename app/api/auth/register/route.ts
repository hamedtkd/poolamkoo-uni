...
import { AccountInputError, createAccountSession, LOCAL_SCOPE_COOKIE, registerAccount, scopeCookieOptions, SESSION_COOKIE, sessionCookieOptions } from "@/lib/server/auth";
...
export async function POST(request: Request) {
  ...
  try {
    const user = await registerAccount(await request.json());
    const session = await createAccountSession(user.id);
    const response = NextResponse.json({ user: { id: user.id, email: user.email, displayName: user.displayName } }, { status: 201 });
    ...
    return NextResponse.json({ error: "ساخت حساب انجام نشد. دوباره تلاش کن." }, { status: 500 });
  }
}
