...
import { transactionExtraCosts } from "@/lib/investment-lots";
...
export function PortfolioTables({ positions, allocationRows, transactions, assets, settings, onTransaction, onEditAsset, onArchiveAsset, onRestoreAsset, onEditTransaction, onDeleteTransaction }: {
  ...
  const positionColumns: ColumnDef<DataTableFeatures, PositionRow, unknown>[] = [
    ...
    { accessorKey: "currentValue", header: "ارزش فعلی", cell: ({ row }) => <CurrentValueCell row={row.original} settings={settings} /> },
  ...
  const transactionColumns: ColumnDef<DataTableFeatures, InvestmentTransaction, unknown>[] = [
    ...
    { accessorKey: "amountToman", header: "مبلغ", cell: ({ row }) => <div><SensitiveValue>{formatMoney(row.original.amountToman, settings.displayUnit)}</SensitiveValue>{transactionExtraCosts(row.original) > 0 && <div className="mt-1 text-[10px] text-muted-foreground">هزینه‌ها: <SensitiveValue>{formatMoney(transactionExtraCosts(row.original), settings.displayUnit, true)}</SensitiveValue></div>}</div> },
...
function PriceCell({ row, settings }: { row: PositionRow; settings: AppSettings }) {
  if (!row.valuationAvailable) return <div><span className="type-caption text-muted-foreground">در دسترس نیست</span><div className="mt-1 text-[9px] text-muted-foreground">موجودی و بهای خرید محلی محفوظ است</div></div>;
  return <div><SensitiveValue>{formatMoney(row.price, settings.displayUnit, true)}</SensitiveValue><PriceProvenance row={row} /></div>;
}

function CurrentValueCell({ row, settings }: { row: PositionRow; settings: AppSettings }) {
  if (!row.valuationAvailable) return <div><span className="type-caption text-muted-foreground">قیمت فعلی نداریم</span><div className="mt-1 text-[10px] text-muted-foreground">بهای خرید باز: <SensitiveValue>{formatMoney(row.cost, settings.displayUnit, true)}</SensitiveValue></div></div>;
  return <SensitiveValue className="type-strong">{formatMoney(row.currentValue, settings.displayUnit)}</SensitiveValue>;
}
...
function AssetMobileCard({ row, allocation, settings, onTx, onEdit, onArchive, onRestore }: { row: PositionRow; allocation?: PortfolioAllocationRow; settings: AppSettings; onTx: () => void; onEdit: () => void; onArchive: () => void; onRestore: () => void }) {
  return <div className="p-4"><div className="flex items-start justify-between gap-3"><div><div className="type-strong">{row.asset.name}</div><div className="mt-1 text-[10px] text-muted-foreground">مقدار {formatNumber(row.qty, 6)}</div></div><div className="text-end"><SensitiveValue className="type-strong">{row.valuationAvailable ? formatMoney(row.currentValue, settings.displayUnit) : formatMoney(row.cost, settings.displayUnit)}</SensitiveValue><div className={cn("mt-1 text-xs type-strong", row.valuationAvailable ? (row.returnPct >= 0 ? "text-profit" : "text-loss") : "text-muted-foreground")}>{row.valuationAvailable ? formatSignedPercent(row.returnPct) : "قیمت فعلی نداریم"}</div></div></div>{allocation && <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-muted/40 p-3 text-[11px]"><span>سهم فعلی <b>{formatPercent(allocation.currentPct, 1)}</b> · هدف <b>{formatPercent(allocation.targetPct, 0)}</b></span><Badge>{allocationStatus[allocation.status]}</Badge></div>}<div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-muted/40 p-3 text-[11px]"><div><span className="text-muted-foreground">میانگین خرید</span><SensitiveValue className="mt-1 type-strong">{formatMoney(row.avgPrice, settings.displayUnit, true)}</SensitiveValue></div><div><span className="text-muted-foreground">قیمت فعلی</span>{row.valuationAvailable ? <><SensitiveValue className="mt-1 type-strong">{formatMoney(row.price, settings.displayUnit, true)}</SensitiveValue><PriceProvenance row={row} /></> : <div className="mt-1 type-caption text-muted-foreground">در دسترس نیست</div>}</div></div>{row.asset.archived ? <div className="mt-3"><Button className="w-full" size="sm" variant="outline" onClick={onRestore}><RiRestartLine /> بازگردانی به سبد</Button></div> : <div className="mt-3 flex flex-wrap gap-1"><Button size="sm" onClick={onTx}><RiExchangeLine /> خرید/فروش</Button><Button size="sm" variant="ghost" onClick={onEdit}><RiEditLine /> ویرایش</Button><Button size="sm" variant="ghost" className="text-destructive" onClick={onArchive}><RiDeleteBin6Line /> آرشیو</Button></div>}</div>;
}
